...
{
    public class Starbucks : CoffeeShop
    {
        public Starbucks(string name, double initialBalance) : base(name, initialBalance) { }
        ...
        public override void MakeOrder(double amount, string orderDetails)
        {
            ...
            Balance -= amount;
            Console.WriteLine($"Order placed at Starbucks: {orderDetails} for {amount}. Remaining balance: {Balance}");
        }

        public override void CancelOrder(double amount)
        {
            Balance += amount;
            Console.WriteLine($"Order cancelled at Starbucks. Refunded {amount}. New balance: {Balance}");
        }
    }
}
